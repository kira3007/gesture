...
//    (function() {
        ...
        var G_vmlCanvasManager_ = {
            ...
            init_: function(doc) {
                ...
                'text-align:left;width:300px;height:150px}' + 'canvas object{width:100%;height:100%;border:0;' + 'background:transparen;margin:0}';
...
//                }
        ...
        function createXamlScriptTag() {
            ...
            var str = '<script type=text/xaml>' + '<Canvas x:Name="root" ' + 'xmlns="http://schemas.microsoft.com/client/2007" ' + 'xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml" ' + 'Width="300" ' + 'Height="150" ' + 'Background="Transparent"> ' + '</Canvas>' + '</script>';
            // Find the id of the writtenscript file.
            var scripts = document.scripts;
            var script = scripts[scripts.length - 1];
            var $script = $(str).insertAfter(script);

            xamlId = $script[0].uniqueID;
            $script[0].id = xamlId;
        }

        function getObjectHtml(fn) {
            return '<object type="application/x-silverlight" >' + '<param name="windowless" value="true">' + '<param name="background" value="transparent">' + '<param name="source" value="#' + xamlId + '">' + '</object>';
        }
        ...
        function translateColor(s) {
            ...
            if (rgbaMatch) {
                ...
                return '#' + dec2hex[Math.floor(Number(parts[3]) * 255)] + dec2hex[Number(parts[0])] + dec2hex[Number(parts[1])] + dec2hex[Number(parts[2])];
            }

            var rgbMatch = /rgb\(([^)]+)\)/gi.exec(s);
            if (rgbMatch) {
                var parts = rgbMatch[1].split(',');
                return '#FF' + dec2hex[Number(parts[0])] + dec2hex[Number(parts[1])] + dec2hex[Number(parts[2])];
            }
        ...
        CanvasGradient_.prototype.createStops_ = function(ctx, brushObj, colors) {
            ...
            for (var i = 0, c; c = colors[i]; i++) {
                ...
                gradientStopCollection.add(create(ctx, '<GradientStop Color="%1" Offset="%2"/>', [color, c.offset]));
            }
        };
        ...
        LinearCanvasGradient_.prototype.createBrush_ = function(ctx) {
            var brushObj = create(ctx, '<LinearGradientBrush MappingMode="Absolute" ' + 'StartPoint="%1,%2" EndPoint="%3,%4"/>', [this.x0_, this.y0_, this.x1_, this.y1_]);
        ...
        CanvasGradient_.prototype.createBrush_ = function(ctx) {
            ...
            var brushObj = create(ctx, '<RadialGradientBrush MappingMode="Absolute" ' + 'GradientOrigin="%1,%2" Center="%3,%4" ' + 'RadiusX="%5" RadiusY="%5"/>', [this.x0_, this.y0_, this.x1_, this.y1_, radius]);
  ...
  // set up externs
   ...
   return {
        init: function(canvases){
            if(canvases){
                if(canvases.length){
                    for(var i = 0, c; c = canvases[i]; i++){
                        G_vmlCanvasManager_.initElement(c);
                    }
                }else{
                    G_vmlCanvasManager_.initElement(canvases);
                }
            }
        },
        G_vmlCanvasManager : G_vmlCanvasManager_,
